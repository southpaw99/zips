...
class Premiumize:
	...
	def acount_info_to_dialog(self):
		...
		try:
			...
			log_utils.log('accountInfo = %s' % accountInfo, __name__, log_utils.LOGDEBUG)

			expires = datetime.strptime(str(accountInfo['premium_until']), FormatDateTime)
			log_utils.log('expires = %s' % expires, __name__, log_utils.LOGDEBUG)

			days_remaining = (expires - datetime.today()).days
			log_utils.log('days_remaining = %s' % days_remaining, __name__, log_utils.LOGDEBUG)
